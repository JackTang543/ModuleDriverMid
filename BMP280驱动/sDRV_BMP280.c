...
int8_t sDRV_BMP280_Init(sDRV_BMP280_Conf_t* _conf){
    //初始化I2C:1KHz
    sBSP_I2C1_Init(10000);
    //读取BMP280里的chip_id寄存器与标准值比较,用于检查通信是否正常
    if(read_reg(ADDR_REG_ID) != VAL_CHIPID){
       return -1;
    }   
    //读取校准寄存器
    sBSP_I2C1M_SeqSendByte(BMP280_ADDR,ADDR_REG_CAL0,I2C_FIRST_FRAME);
    delay(1);
    sBSP_I2C1M_SeqReadBytes(BMP280_ADDR,bmp280.cal_val,26,I2C_LAST_FRAME);
    delay(26);
    //复位BMP280
...
/**
  ...
  */
int8_t sDRV_BMP280_GetMeasure(){
    //读取忙标志
    if(op_reg(ADDR_REG_STATUS,REG_STATUS_MSK_MEASURING,OPER_GET_REG) == 1){return -1;}
    
    sBSP_I2C1M_SeqSendByte(BMP280_ADDR,ADDR_REG_PRESS_MSB,I2C_FIRST_FRAME);
    delay(1);
    sBSP_I2C1M_SeqReadBytes(BMP280_ADDR,&(bmp280.press_msb),6,I2C_LAST_FRAME);
    delay(6);
